...
def run_cem_maker(user_inputs: dict) -> dict:
    """
    Generate a base Customer Engagement Map (CEM) using provided user inputs.
    Returns a structured dictionary to be passed to the conductor.
    """

    # 1. Create user input summary for prompt
    summary = f"""
...
Key KPIs: {', '.join(user_inputs['kpis'])}
Preferred Channels: {', '.join(user_inputs['preferred_channels'])}
Tools Used: {', '.join(user_inputs['tools'])}
Team Roles: {user_inputs['team_roles']}

Personas: {user_inputs.get('personas', '')}
Content Types: {user_inputs.get('content_types', '')}
Challenges: {user_inputs.get('challenges', '')}
    """
...
Your job is to guide small SaaS teams with limited budgets through meaningful, scalable engagement strategies.
    """
